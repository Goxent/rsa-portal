...
const TasksPage: React.FC = () => {
    ...
    const injectPhaseSubtasks = useCallback((
        ...
        if (!template?.subtaskDetails?.length) return null;

        const phaseSubtasks = template.subtaskDetails.filter(s => s.phase === newPhase);
    ...
    const swapPhaseChecklist = useCallback((
        ...
        if (!task.taskType) return task.subtasks ?? [];

        const phaseItems = TASK_TYPE_CHECKLISTS[task.taskType]?.[newPhase as AuditPhase] ?? [];

        // Keep manual subtasks, remove old auto-generated ones
        const manualSubtasks = (task.subtasks ?? []).filter(st => !st.isAutoGenerated);

        // Create new auto-generated subtasks for the new phase
        const newAutoSubtasks: SubTask[] = phaseItems.map(item => ({
            id: `st-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
            title: item.title,
            isCompleted: false,
        ...
        return [...manualSubtasks, ...newAutoSubtasks];
    }, []);
    ...
    const injectStatusSubtasks = useCallback((
        ...
        if (!task.templateId) return null;
        const template = templates.find(t => t.id === task.templateId);
        const statusItems = template?.statusSubtasks?.[newStatus];
