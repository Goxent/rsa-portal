...
const TasksPage: React.FC = () => {
  ...
  const KanbanColumn = ({ status, title, colorClass, badgeColor }: { status: TaskStatus, title: string, colorClass: string, badgeColor: string }) => {
      ...
      return (
          <div className="flex-1 min-w-[320px] flex flex-col h-full max-h-[calc(100vh-240px)] animate-fade-in-up">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 px-1 pb-4">
                  {columnTasks.map((task, idx) => {
                      ...
                      <div 
                        ...
                        className="glass-panel p-5 rounded-xl hover:border-brand-500/40 hover:bg-navy-700 transition-all cursor-pointer group relative overflow-hidden shadow-lg border border-white/5 animate-fade-in-up transform hover:-translate-y-1"
                ...
                {filteredTasks.map((task, idx) => {
                    ...
                    <tr key={task.id} onClick={() => handleOpenEdit(task)} className="hover:bg-white/5 transition-colors cursor-pointer group animate-fade-in-up" style={{ animationDelay: `${idx * 50}ms` }}>
                        ...
                        <td className="px-6 py-4 font-mono text-xs">
                            ...
                            <div className="text-white">Due: {task.dueDate}</div>
                        </td>
                      ...
                      <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                           ...
                           <div className="grid grid-cols-2 gap-4">
                               ...
                               <div><label className="block text-sm font-medium text-gray-300 mb-1">Due Date <span className="text-red-400">*</span></label><input type="date" className="w-full glass-input rounded-lg px-3 py-2 text-sm" value={currentTask.dueDate} onChange={(e) => setCurrentTask({...currentTask, dueDate: e.target.value})} disabled={!hasEditPermission}/></div>
                          </div>
