...
const startMarker = '            {/* ── Top bar ── */}';
// Look for the export default line
const endMarker = 'export default AuditDocumentationPage;';

const startIndex = code.indexOf(startMarker);
const endIndexRegex = /(}\s*\);\s*};\s*export default AuditDocumentationPage;)/;
const match = code.match(endIndexRegex);
...
if (startIndex !== -1 && match) {
    ...
    const finalCode = codeBefore + newTopBarAndContent + '\n' + codeAfter;
    fs.writeFileSync('pages/AuditDocumentationPage.tsx', finalCode);
    console.log('Part 2 Top Bar & Grid replaced successfully');
} else {
    console.log('ERROR: Start index or end index not found');
}
