...
export const clientSchema = z.object({
    name: z.string().min(2, 'Company Name is required').max(200, 'Name is too long'),
    email: z.string().email('Invalid email address').optional().or(z.literal('')),
    phone: z.string().min(7, 'Phone number must be at least 7 digits').optional().or(z.literal('')),
    address: z.string().optional(),
    serviceType: z.string().min(1, 'Service type is required').optional(), // Optional in new form, required in legacy
    pan: z.string().optional(),
    taxId: z.string().optional(), // Alias for PAN in some contexts
    industry: z.string().optional(),
    businessType: z.string().optional(),
    riskCategory: z.enum(['Low', 'Medium', 'High']).optional(),
    previousAuditReportLink: z.string().url('Invalid URL format').optional().or(z.literal('')),
...
export const taskSchema = z.object({
    ...
    status: z.enum(['TODO', 'IN_PROGRESS', 'IN_REVIEW', 'COMPLETED', 'BLOCKED']), // Match types.ts TaskStatus
...
export const leaveRequestSchema = z.object({
    ...
    leaveType: z.enum(['SICK', 'CASUAL', 'ANNUAL', 'EARNED', 'UNPAID', 'OTHER']), // Consolidated enum
...
// Simplified schema for form binding if needed
