...
const SidebarItem = ({ to, icon: Icon, label }: { to: string, icon: any, label: string }) => {
  return (
    <NavLink 
      to={to} 
      className={({ isActive }) => 
        `flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-300 mb-1.5 border border-transparent group relative overflow-hidden ${
          isActive 
...
const Layout: React.FC = () => {
  ...
  const loadNotifications = async () => {
    ...
    // 2. Check Tasks (Mocking simple count)
    const tasks = await AuthService.getAllTasks();
    const myTasks = tasks.filter(t => t.assignedTo.includes(user.uid) && t.status !== 'COMPLETED');
    if (myTasks.length > 0) {
      notifs.push({
        id: 'task_info',
        title: 'Pending Tasks',
        message: `You have ${myTasks.length} tasks assigned to you pending completion.`,
        type: 'INFO',
        read: false,
        createdAt: new Date().toISOString()
      });
