...
const SearchableSelect: React.FC<SearchableSelectProps> = ({
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (target.closest('[data-portal-content="generic-select"]')) return;
            setIsOpen(false);
            setSearchTerm('');
        };
    ...
    const dropdownContent = (
        <div 
            data-portal-content="generic-select"
        ...
        >
            <div className="bg-[#0d1526] border border-white/10 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden backdrop-blur-xl">
                {/* Search Bar */}
                ...
                <div className="max-h-64 overflow-y-auto custom-scrollbar py-1">
                    {filteredItems.length > 0 ? (
                        ...
                        <div className="p-4 text-center">
                            <Search size={24} className="mx-auto text-gray-600 mb-2 opacity-20" />
                            <p className="text-[10px] text-gray-500">No items found</p>
                        </div>
                    )}
                </div>
            </div>
    ...
    return (
        <div className={`relative w-full ${isOpen ? 'z-[100]' : ''} ${className}`} ref={wrapperRef}>
            {/* Trigger Button */}
            <button
                type="button"
            ...
            >
                <div className="flex items-center gap-2 min-w-0">
                    {selectedItem?.icon ? (
                        ...
                        <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${selectedItem ? 'bg-brand-400' : 'bg-gray-600'}`} />
                    )}
                    <span className={`truncate font-medium ${selectedItem ? 'text-white' : 'text-gray-500'}`}>
