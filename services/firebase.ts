...
export const AuthService = {
    ...
    getAllStaff: async (): Promise<UserProfile[]> => {
        // Optimization: Filter for Active STAFF and MANAGERS on server.
        // This excludes all ADMIN and MASTER_ADMIN roles entirely at the source.
        const q = query(
            collection(db, 'users'), 
            where('status', '==', 'Active'),
    ...
    clockIn: async (userId: string, method: 'WEB' | 'MOBILE' = 'WEB', location?: string, notes?: string, workLogs?: WorkLog[], clientIds?: string[]) => {
        ...
        // Check if there is a missing clock out on a past day
        ...
        try {
            ...
            const pastRecords = snapRecent.docs.map(d => d.data() as AttendanceRecord);
            const lastPastRecord = pastRecords.find(r => r.date < dateStr);
            if (lastPastRecord && lastPastRecord.clockIn && !lastPastRecord.clockOut) {
                if (!finalNotes.toLowerCase().includes('clock out missing')) {
                    finalNotes = finalNotes ? `clock out missing. ${finalNotes}` : 'clock out missing';
                }
            }
        } catch (e) {
            console.warn("Failed to check past attendance for missing clock out", e);
        }
